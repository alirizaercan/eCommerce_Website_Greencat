-- Admin Type Table (Önce bağımlılığı olan tabloları oluşturuyoruz)
...
-- Order Details Table (payment_id alanını daha sonra ALTER TABLE ile ekleyeceğiz)
CREATE TABLE order_details (
    order_id BIGSERIAL PRIMARY KEY,
    customer_id BIGINT NOT NULL REFERENCES customer(customer_id) ON DELETE CASCADE,
    total DECIMAL(10, 2) NOT NULL,
    tax DECIMAL(10, 2) DEFAULT 0.00,
    order_status VARCHAR(50) NOT NULL, -- VARHCAR yazım hatası düzeltildi
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    modified_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);
...
-- Order Details tablosuna payment_id alanını ekleme
ALTER TABLE order_details 
ADD COLUMN payment_id BIGINT REFERENCES payment_details(payment_id) ON DELETE SET NULL;
